...
def blog() -> str:
    ...
    return render_template('blog.html', posts=posts, title=BBL('Blog'))
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(BBL('Your post has been created!'), 'alert-success')
        return redirect(url_for('posts.post', post_id=post.id))
    return render_template('create_post.html', title=BBL('New Post'), form=form, legend='New Post')
...
def update_post(post_id: float):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(BBL('Your post has been updated!'), 'alert-success')
        return redirect(url_for('posts.post', post_id=post.id))
    elif request.method == 'GET':
        form.title.data = post.title
        form.content.data = post.content
    return render_template('create_post.html', title=BBL('Update Post'), form=form, legend=BBL('Update Post'))
...
def delete_post(post_id: float) -> str:
    ...
    db.session.commit()
    flash(BBL('Your post has been deleted!'), 'alert-success')
